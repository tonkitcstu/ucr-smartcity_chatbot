...
async def _turn(
    ...
    #
    # ทางบังคับให้พูดมีอยู่แล้วในบล็อก else ข้างบน — เดิมใช้เฉพาะตอนโมเดลวน tool
    # ไม่หยุด ทั้งที่อาการ "ไม่ได้ข้อความ" เหมือนกันเป๊ะ ใช้ซ้ำเลย
    if not text and not begged:
        forced, spent = await llm.chat(messages)
        used = _add(used, spent)
        text, asking_location, asking_photo = _buttons(forced.strip())
