...
async def _turn(
    ...
    used = {"calls": 0, "tokens": 0}
    begged = False   # ขอร้องให้พูดไปแล้วหรือยัง — ขอซ้ำสองรอบไม่ได้ ดูตรงที่เช็ค text
    ...
    # **ขอใหม่หนึ่งครั้งก่อนยอมแพ้** — เดิมตรงนี้ตกลงคำขอโทษทันที
    #
    # นับจาก transcript จริง 9 ชุด (44 ตาของบอท) เมื่อ 9 ส.ค. 69: ชาวบ้านเจอ
    # คำขอโทษ 3 ตา = **1 ใน 3 ของบทสนทนาเจออย่างน้อยหนึ่งครั้ง** และมันไปตกตรง
    # ตาที่แย่ที่สุดเท่าที่จะแย่ได้ — ใบ 6 เขาเพิ่งพิมพ์ว่า "เด็กนักเรียนกับคนแก่
    # เดินผ่านตรงนั้นทุกเย็นเลยค่ะ" ซึ่งคือประโยคที่ทั้งโปรเจกต์นี้มีไว้เก็บ
    # แล้วบอทตอบกลับว่าขอโทษ ช่วยเล่าอีกครั้ง
    #
    # กลไกหนึ่งที่พิสูจน์ได้: `_buttons()` เป็นคนลบข้อความเอง ถ้าโมเดลตอบมาแต่
    # ป้าย `[ปุ่ม: ตำแหน่ง]` ล้วน ๆ ข้อความที่เหลือคือสตริงว่าง (ยังไม่ยืนยันว่า
    # ทั้งสามตาที่เจอเกิดจากทางนี้ แต่ทางนี้ทำให้เกิดได้จริงแน่)
    #
    # ยืมทางบังคับให้พูดจากบล็อก else ข้างบนมาใช้ **แต่ไม่ใช่ของเดียวกัน** ที่นั่น
    # `messages` โตขึ้นแล้วจาก `tool_exchange()` โมเดลได้ของใหม่ไปคิด ส่วนตรงนี้
    # `break` ข้ามบรรทัดนั้นไป และไม่มีใครต่อคำตอบว่างเข้า `messages`
    # **input จึงเหมือนเดิมทุกตัวอักษร — มันคือการทอยใหม่ ไม่ใช่การขอใหม่**
    # ใช้ได้เพราะ sampling ไม่ตายตัวเท่านั้น ทอยได้ครั้งเดียว ซ้ำอีกก็เท่าเดิม
    #
    # **ธงปุ่มของรอบแรกห้ามหาย** ตาที่โมเดลตอบมาแต่ป้าย `[ปุ่ม: ตำแหน่ง]` คือตาที่
    # มันขอปุ่มไว้ถูกแล้ว แค่ลืมพูด ถ้าเอาผลรอบสองทับตรง ๆ ธงจะกลายเป็น False
    # (ยิงเทสแล้วเป็นจริง) แล้วชาวบ้านได้ข้อความที่ดีขึ้นแต่ไม่มีปุ่มให้กด
